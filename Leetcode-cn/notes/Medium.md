...
- [[179] Largest Number 最大数](#179-largest-number-最大数)
- [[200] 岛屿数量](#200-岛屿数量)
...
1. Solution1, 递归DFS，Time: O(n), Space: O(1), Runtime: 38%
   - 函数返回的是root的最近祖先节点
   - 判断root是否是最近公共祖先(root == p or q)，不等的话对左右子结点调用函数，分别获取左右子结点的最近祖先，再判断两者是否为公共祖先，不是的话继续递归
      - 如果left存在，right不存在，即公共祖先在左子树中
      - 如果right存在，left不存在，即公共祖先在右子树中
      - 否则root为公共祖先
